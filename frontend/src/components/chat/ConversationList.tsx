  ...
  if (loading) {
    return (
      <div className="p-3 space-y-2">
        {[1, 2, 3].map((i) => (
          <div key={i} className="animate-pulse h-12 rounded-lg bg-surface-light" />
        ))}
      </div>
    );
  }
        ...
        return (
          <div key={entry.id} className="relative group/item">
            {renamingId === entry.id ? (
              <div className="px-2 py-1.5">
                <input
                  ...
                  className="w-full bg-surface-light text-zinc-100 rounded px-2 py-1 text-xs outline-none ring-1 ring-violet-500/50"
                />
              </div>
            ) : (
              <Link
              ...
              >
                ...
                <button
                  ...
                  className="opacity-0 group-hover/item:opacity-100 shrink-0 p-1 rounded hover:bg-border text-zinc-500 hover:text-zinc-300 transition-all"
                >
            ...
            {menuId === entry.id && (
              <div
                ref={menuCallbackRef}
                className={`absolute right-2 z-20 w-36 rounded-lg border border-border bg-surface py-1 shadow-xl ${
              ...
              >
                <button
                  ...
                  className="flex w-full items-center gap-2 px-3 py-1.5 text-xs text-zinc-300 hover:bg-surface-light transition-colors"
                >
                  <Pencil size={12} /> Rename
                </button>
                <button
                  onClick={() => handleDelete(entry.id)}
                  className="flex w-full items-center gap-2 px-3 py-1.5 text-xs text-red-400 hover:bg-surface-light transition-colors"
                >
